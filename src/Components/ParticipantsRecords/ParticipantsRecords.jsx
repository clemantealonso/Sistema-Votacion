...
class ParticipantsRecords extends Component
{
    ...
    Eliminar(Id,Ruta)
    {
        if(window.confirm('¿Esta seguro de que quiere eliminar este registro?'))
            fire.database().ref(Ruta).child(Id).remove()
    }
    render()
    {
        return(<>
            <div className="row">
                ...
                <div className="col-3">
                    <div className="mr-2">
                        <button type="submit" onClick={()=>this.Eliminar(this.props.Ruta,this.props.participantId)} className="btn btn-danger">Eliminar</button>
                    </div>
